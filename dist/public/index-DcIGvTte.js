const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./layout-V1jJrGtE.js","./vue-BjL5XjkG.js","./vue-CIUEyO5u.css","./bootstrap-IS5Me-CY.js","./bootstrap-CVQV2YQR.css","./layout-rJVpGJdd.css","./explorer-DYwjYzWl.js","./explorer-C88R5OXZ.css"])))=>i.map(i=>d[i]);
...
function AxiosError$1(message, code, config2, request2, response) {
  Error.call(this);
...
AxiosError$1.from = (error, code, config2, request2, response, customProps) => {
  ...
  AxiosError$1.call(axiosError, msg, errCode, config2, request2, response);
...
const service = axios$1.create({
  ...
  // 设置统一的请求前缀// http://localhost:8888
  timeout: 3e4
  // 设置统一的超时时长
});
service.interceptors.request.use(
  (config2) => {
    const pendingKey = getPendingKey(config2);
...
function getPendingKey(config2) {
  const { url, method, params } = config2;
  let { data } = config2;
...
function abortAllPending() {
  for (const [key2, value] of pendingMap.entries()) {
    value.abort();
...
function getHttpErrorMsg(error) {
  ...
  if (axios$1.isCancel(error)) {
    console.error(
      `重复请求：${error.config.url} ${error.message}`
    );
  }
...
class JEventEmitter extends EventEmitter {
  ...
  // 获取或生成订阅器
  static getEmitter(key2) {
    let emmiter = this.emitters.get(key2);
    if (!emmiter) {
      emmiter = new JEventEmitter();
      emmiter.key = key2;
      this.emitters.set(key2, emmiter);
    }
    return emmiter;
  }
  // 释放
  static removeEmitter(key2) {
    if (this.emitters.has(key2)) {
      return this.emitters.delete(key2);
    }
    return false;
  }
}
class Entity extends JEventEmitter {
  /**
   * 状态
   * @public
   */
...
const sysConfig = new SysConfig(config$1.state);
let activePinia;
...
const useSessionStore = /* @__PURE__ */ defineStore("session", {
  ...
  getters: {
    // 是否登录
    isLoggedIn: (state2) => {
      console.log("Recomputing isLoggedIn", state2, state2.id, state2.userId);
      return !!state2.id && !!state2.userId;
    },
    // 获取用户名
    userName: (state2) => state2.user?.name || "",
    // 获取登录ID
    userLoginId: (state2) => state2.loginId,
    // 获取用户头像
    userAvatar: (state2) => state2.user?.avatar || "",
    // 登陆token
    token: (state2) => state2.id || ""
  },
  actions: {
    // 设置登录状态
    setSession(session) {
      this.$patch((state2) => {
        Object.assign(state2, session);
      });
    },
    // 清除登录状态（退出登录）
    clearSession() {
    ...
    updateUserInfo(user) {
      if (this.user) {
        this.$patch((state2) => {
          state2.user = Object.assign(state2.user || {}, user);
        });
      }
    }
  },
...
const GlobalEventBus = new JEventEmitter();
function publish(name, data) {
  return GlobalEventBus.emit(name, data);
}
var define_process_env_default = {};
const isNWjs = typeof process !== "undefined" && process.__nwjs !== void 0;
const isVSCode = typeof acquireVsCodeApi !== "undefined" || typeof process !== "undefined" && define_process_env_default.VSCODE_PID;
const isChromeExtension = !isNWjs && !isVSCode && (() => {
  try {
...
const isElectron = typeof process === "object" && process.versions?.electron !== void 0;
const isBrowser = typeof window !== "undefined" && !isNWjs && !isVSCode && !isChromeExtension && !isElectron;
const UA = isBrowser ? navigator.userAgent : "";
UA.match(/QQ\/([\d.]+)/i) && UA.match(/QQ\/([\d.]+)/i)?.[1];
UA.match(/MicroMessenger\/([\d.]+)/i) && UA.match(/MicroMessenger\/([\d.]+)/i)?.[1]?.split(".").slice(0, 3).join(".");
UA.match(/Web[kK]it[/]{0,1}([\d.]+)/);
const android = UA.match(/(Android);?[\s/]+([\d.]+)?/);
!!UA.match(/\(Macintosh; Intel /);
const ipad = UA.match(/(iPad).*OS\s([\d_]+)/), ipod = UA.match(/(iPod)(.*OS\s([\d_]+))?/), iphone = !ipad && UA.match(/(iPhone\sOS)\s([\d_]+)/);
UA.match(/(webOS|hpwOS)[\s/]([\d.]+)/);
UA.match(/(BlackBerry).*Version\/([\d.]+)/);
UA.match(/(BB10).*Version\/([\d.]+)/);
UA.match(/PlayBook/);
const chrome = UA.match(/Chrome\/([\d.]+)/) || UA.match(/CriOS\/([\d.]+)/), x5 = UA.match(/MQQBrowser\/([\d.]+)/i);
...
function getRequestUrl(api2) {
  if (/^(http(s)?:)?\/\//.test(api2)) return api2;
  const apiUrl2 = config$1.apiUrl || `${location.protocol}//${location.hostname}${[80, 443].includes(Number(location.port)) ? "" : ":" + location.port}`;
...
async function requestServer(url, data, option) {
  if (isNWjs()) {
    ...
    const res = await request$1(url, data, option);
    return res;
  }
}
...
const routes = [
  {
    path: "/database",
    component: () => __vitePreload(() => import("./layout-V1jJrGtE.js"), true ? __vite__mapDeps([0,1,2,3,4,5]) : void 0, import.meta.url),
    ...
    children: [
      {
        path: "index",
        name: "database-index",
        component: () => __vitePreload(() => import("./explorer-DYwjYzWl.js"), true ? __vite__mapDeps([6,1,2,3,4,7]) : void 0, import.meta.url),
        meta: {
          title: "数据库管理首页",
          icon: "bi-house",
          needAuth: false
...
router.beforeEach(async (to, from, next) => {
  abortAllPending();
  const ret = await initLoginState();
  if (ret === false) return;
  if (to.meta?.needAuth) {
    await checkLogin();
  }
...
function hydrateStore(store, { storage: storage2, serializer, key: key2, debug, pick, omit, beforeHydrate, afterHydrate }, context, runHooks = true) {
  try {
    if (runHooks) beforeHydrate?.(context);
    const fromStorage = storage2.getItem(key2);
    if (fromStorage) {
...
function persistState(state2, { storage: storage2, serializer, key: key2, debug, pick, omit }) {
  try {
    const picked = pick ? deepPick(state2, pick) : state2;
    const omitted = omit ? deepOmit(picked, omit) : picked;
    const toStorage = serializer.serialize(omitted);
    storage2.setItem(key2, toStorage);
  } catch (error) {
    if (debug) console.error("[pinia-plugin-persistedstate]", error);
  }
}
...
function createPersistence(context, optionsParser, auto) {
  ...
  persistences.forEach((p) => {
    ...
    store.$subscribe((_mutation, state2) => persistState(state2, p), { detached: true });
  });
}
...
const _hoisted_1$4 = { class: "toast-container position-fixed top-0 start-50 translate-middle-x p-3" };
const _hoisted_2$4 = {
  key: 0,
  class: "toast-header"
};
const _hoisted_3$3 = { class: "me-auto" };
const _hoisted_4$2 = ["onClick"];
const _hoisted_5$2 = { class: "toast-body" };
...
const _sfc_main$3 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    return (_ctx, _cache) => {
      return __props.isLoading ? (openBlock(), createElementBlock("div", _hoisted_1$3, [
        _cache[0] || (_cache[0] = createBaseVNode("div", { class: "loading-spinner" }, [
          createBaseVNode("div", {
...
const Loading = /* @__PURE__ */ _export_sfc(_sfc_main$3, [["__scopeId", "data-v-48d1b926"]]);
const _hoisted_1$2 = {
...
const _hoisted_4$1 = { class: "d-flex align-items-center mb-3" };
const _hoisted_5$1 = { class: "flex-grow-1 text-wrap" };
const _hoisted_6$1 = {
  key: 0,
...
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  props: {
    ...
    teleport: {
      type: [Boolean, String],
      default: true
      // true表示传送到body，也可传具体选择器如"#app"
    }
  },
  emits: ["onConfirm", "onClose", "onCancel", "onHidden"],
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    const typeIcon = computed(() => {
      ...
      return iconMap[dynamicType.value] || "";
    });
    const headerClass = computed(() => {
      ...
      return classMap[dynamicType.value] || "";
    });
    ...
    const confirmButtonClass = computed(() => {
      ...
      return classMap[dynamicType.value] || "btn-primary";
    });
    const originalParent = ref(null);
    const getTargetContainer = () => {
      if (!props.teleport) return null;
      if (typeof props.teleport === "string") {
        return document.querySelector(props.teleport) || null;
      } else {
    ...
    function getModal() {
      ...
      const m2 = new Modal$1(modalContainer.value, {
        backdrop: "static",
        keyboard: false
      });
    ...
    onUnmounted(() => {
      if (modalContainer.value) {
        modalContainer.value.removeEventListener(
          "hidden.bs.modal",
          modalContainer.value.handleModalHide
        );
        modalContainer.value.removeEventListener(
          "show.bs.modal",
          modalContainer.value.handleModalShow
        );
      }
      if (modal) {
        modal.dispose();
        ...
        createBaseVNode("div", {
          ...
          createBaseVNode("div", {
            ...
            createBaseVNode("div", {
              ...
              createBaseVNode("h5", _hoisted_1$2, [
                typeIcon.value ? (openBlock(), createElementBlock("i", {
                  key: 0,
                  class: normalizeClass([typeIcon.value, "me-2"])
                }, null, 2)) : createCommentVNode("", true),
                createTextVNode(" " + toDisplayString(dynamicTitle.value || props.title), 1)
              ]),
              createBaseVNode("button", {
                type: "button",
                class: "btn-close",
            ...
            modalShow.value ? (openBlock(), createElementBlock("div", _hoisted_2$2, [
              _ctx.$slots.default ? renderSlot(_ctx.$slots, "default", { key: 0 }, void 0) : (openBlock(), createElementBlock("div", _hoisted_3$2, [
                createBaseVNode("div", _hoisted_4$1, [
                  typeIcon.value ? (openBlock(), createElementBlock("i", {
                    key: 0,
                    class: normalizeClass([typeIcon.value, "me-3 fs-1"])
                  }, null, 2)) : createCommentVNode("", true),
                  createBaseVNode("div", _hoisted_5$1, toDisplayString(dynamicContent.value), 1)
                ]),
                shouldShowDetails() ? (openBlock(), createElementBlock("div", _hoisted_6$1, [
                  createBaseVNode("button", {
                    class: "btn btn-sm btn-outline-secondary",
                    onClick: toggleDetails
                  }, [
                    createTextVNode(toDisplayString(detailsExpanded.value ? "隐藏" : "详情") + " ", 1),
            ...
            createBaseVNode("div", _hoisted_9, [
              _ctx.$slots.footer ? renderSlot(_ctx.$slots, "footer", { key: 0 }, void 0) : (openBlock(), createElementBlock(Fragment, { key: 1 }, [
                dynamicShowCancel.value || props.closeButton.show ? (openBlock(), createElementBlock("button", {
                  ...
                  onClick: cancel
                }, toDisplayString(dynamicCancelText.value || props.closeButton.text), 1)) : createCommentVNode("", true),
                props.confirmButton.show ? (openBlock(), createElementBlock("button", {
                  key: 1,
                  type: "button",
                  class: normalizeClass(["btn", confirmButtonClass.value]),
                  onClick: confirm
                }, toDisplayString(dynamicConfirmText.value || props.confirmButton.text), 3)) : createCommentVNode("", true)
              ], 64))
            ]),
            createVNode(Loading, {
...
const Modal = /* @__PURE__ */ _export_sfc(_sfc_main$2, [["__scopeId", "data-v-0f64e462"]]);
const _hoisted_1$1 = {
  "aria-label": "Page navigation",
  class: "px-3"
};
const _hoisted_2$1 = { class: "pagination" };
const _hoisted_3$1 = ["onClick", "tabindex"];
  ...
  setup(__props, { emit: __emit }) {
    ...
    const visiblePages = computed(() => {
      ...
      if (props.totalPages <= maxVisiblePages) {
        for (let i2 = 1; i2 <= props.totalPages; i2++) {
          pages.push(i2);
        }
        return pages;
      }
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("nav", _hoisted_1$1, [
        createBaseVNode("ul", _hoisted_2$1, [
          createBaseVNode("li", {
            class: normalizeClass(["page-item", { disabled: __props.currentPage === 1 }])
          }, [
            createBaseVNode("a", {
              class: "page-link",
              href: "#",
          ...
          (openBlock(true), createElementBlock(Fragment, null, renderList(visiblePages.value, (page) => {
            return openBlock(), createElementBlock("li", {
              key: page,
              class: normalizeClass(["page-item", { disabled: page === "...", active: __props.currentPage === page }])
            }, [
              createBaseVNode("a", {
                class: "page-link",
                href: "#",
          ...
          createBaseVNode("li", {
            class: normalizeClass(["page-item", { disabled: __props.currentPage === __props.totalPages }])
          }, [
            createBaseVNode("a", {
              class: "page-link",
              href: "#",
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", _hoisted_1, [
                ...
                return openBlock(), createElementBlock("tr", {
                  ...
                  (openBlock(true), createElementBlock(Fragment, null, renderList(props.columns, (column) => {
                    return openBlock(), createElementBlock("td", {
                      ...
                      renderSlot(_ctx.$slots, column.name || column, {
                        row,
                        column
                      }, () => [
        ...
        props.showPagination ? (openBlock(), createBlock(_sfc_main$1, {
          key: 0,
...
const DataGrid = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-f1c2cf28"]]);
...
const modalPlugin = {
  install(app2) {
    ...
    app2.config.globalProperties.$modal = modalInstance = {
      ...
      // 新增方法
      alert(options) {
        const opts = typeof options === "string" ? { content: options, type: "info", showCancel: false } : { ...options, showCancel: false };
        return showModal(opts);
      },
      confirm(options) {
        const opts = typeof options === "string" ? { content: options, type: "warning" } : { ...options, showCancel: true };
        return showModal(opts);
      },
      success(content) {
        return this.alert({ content, type: "success", confirmText: "确定" });
      },
      error(content, details) {
        return this.alert({ content, type: "error", confirmText: "确定", details });
      },
      warning(content) {
        return this.alert({ content, type: "warning", confirmText: "确定" });
      },
      info(content) {
        return this.alert({ content, type: "info", confirmText: "确定" });
      }
    };
  },
  get() {
    const instance2 = getCurrentInstance();
    if (!instance2) return null;
    const modal = instance2.appContext.config?.globalProperties?.$modal;
    return modal;
  }
};
...
const showAlert = (content, type2 = "info", details) => {
  return showModal({
    content,
    type: type2,
    showCancel: false,
...
const showConfirm = (content, options) => {
  return showModal({
    title: "确认",
    content,
    type: "warning",
