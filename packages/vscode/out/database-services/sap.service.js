...
exports.SAPHANADatabaseService = void 0;
 ...
 */
class SAPHANADatabaseService extends base_service_1.BaseDatabaseService {
    getDatabaseType() {
        return 'sap';
    }
    /**
    ...
    async getColumns(dataSource, database, table) {
      ...
      `, [database, table]);
            const primaryKeys = new Set(primaryKeyResult.map((row) => row.COLUMN_NAME));
            return result.map((row) => ({
                name: row.name,
                type: row.type,
    ...
    async getIndexes(dataSource, database, table) {
        ...
        ORDER BY 
          INDEX_NAME
      `, [database, table]);
            const indexDetails = await Promise.all(result.map(async (index) => {
        ...
        `, [database, table, index.name]);
                return {
                    ...
                    columns: columnsResult.map((col) => col.COLUMN_NAME),
...
exports.SAPHANADatabaseService = SAPHANADatabaseService;
